...
1. **Vision Backbone**: ResNet-18 or a DINOv2 / DINOv3 ViT processes images from multiple camera viewpoints
...
### Using a DINOv2 / DINOv3 vision backbone

Requires `pip install 'lerobot[transformers-dep]'`.

Supported `--policy.vision_backbone` `model_type` values:

- `dinov2` (`facebook/dinov2-small`)
- `dinov2_with_registers` (`facebook/dinov2-with-registers-small`)
- `dinov3_vit` (`facebook/dinov3-vits16-pretrain-lvd1689m`; gated)
...
- `--policy.backbone_token_budget`: 300 (default). `null`: native DINO grid.
- `--policy.freeze_vision_backbone=true`: freeze the vision backbone.

```bash
lerobot-train \
  --dataset.repo_id=${HF_USER}/your_dataset \
  --policy.type=act \
  --policy.vision_backbone=facebook/dinov2-small \
  --policy.backbone_token_budget=300 \
  --policy.freeze_vision_backbone=true \
  --policy.device=cuda \
  --output_dir=outputs/train/act_dinov2_your_dataset
```
