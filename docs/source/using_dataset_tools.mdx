...
`lerobot-dataset-plot` saves one PNG per episode. Each figure shows camera frames along the top and the matching state or action traces below. By default it plots every episode and every RGB video camera.
...
Use `--episodes` and `--image-observations` to plot a subset. Figures are written to `<dataset-path>/episode_plots/episode_<i>.png` unless `--output-dir` is set.
