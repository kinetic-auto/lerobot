...
**From a local folder:**
Point `--root` at the dataset directory (the folder that contains `meta/`). `--repo-id` is optional.
...
Once executed, the tool opens the Rerun web viewer and displays the camera streams, robot states, and actions for the selected episode.
...
## Static episode overview

`lerobot-dataset-plot` saves one PNG per episode. Each figure shows camera frames along the top and the matching state or action traces below. By default it plots every episode, every RGB video camera, and `observation.state` plus `action`.
...
Use `--episodes`, `--image-observations`, `--feature-keys`, and `--obs-plot-groups` to plot a subset. Figures are written to `<dataset-path>/episode_plots/episode_<i>.png` unless `--output-dir` is set.
